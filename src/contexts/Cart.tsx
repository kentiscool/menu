...
const CartContext = React.createContext<{state: Cart; dispatch: Dispatch} | undefined>(undefined)
...
function CartProvider({ children }: Props) {
    ...
    const [state, dispatch] = React.useReducer<React.Reducer<Cart, Action>>(CartReducer, initialState);
...
class AddItemAction { 
    ...
    _reduce(cart: Cart): Cart {
        ...
        let newOrders = cart.orders.map(order => {
            if (order.item === this.order.item && order.preference === this.order.preference) {
...
class RemoveItemAction { 
    ...
    _reduce(cart: Cart): Cart {
        let newOrders: Order[] = cart.orders.map(order => {
            if (order.item === this.order.item && order.preference === this.order.preference) {
                order.quantity -= this.order.quantity;
            }
